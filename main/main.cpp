...
uint8_t arr[8] = {0x00, 0x00, 0x00, 0x00, 0x55, 0x00, 0x00, 0x55};
uint8_t receivedArr[4] = {0};
...
extern "C" void app_main() {
    ...
    spi_bus_config_t buscfg = {.mosi_io_num = PIN_NUM_MOSI,
                               ...
                               0,
                               0};
    spi_device_interface_config_t devcfg = {//.clock_speed_hz=26*1000*1000,           //Clock out at 26 MHz
                                            .command_bits = 0,
                                            .address_bits = 0,
                                            .dummy_bits = 0,
                                            .mode = 0,                  // SPI mode 0
    ...
    uint8_t *buffer = reinterpret_cast<uint8_t*>(heap_caps_malloc(32, MALLOC_CAP_DMA)); // for DMA transaction

    spi_transaction_t t;
    memset(&t, 0, sizeof(t));
    t.length = 4 * 8;
    t.rxlength = 4 * 8;
    t.tx_buffer = reinterpret_cast<const void*>(arr);
    t.rx_buffer = reinterpret_cast<void*>(receivedArr);
